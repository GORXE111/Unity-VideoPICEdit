...
namespace Love.EditorTools
{
    ...
    ///
    /// 已验证：ILCE-7RM4A 未压缩 14bit，和机内 JPEG 预览逐通道比对，
    /// R/G 与 B/G 比值误差在 2% 以内（剩下的差异是机内那条创意曲线）。
    /// </summary>
    public static class SonyRawImporter
    {
        ...
        public static Result Load(string path, Options o)
        {
            ...
            // 目前只解未压缩的。压缩 ARW（有损 32767 / 无损）用的是索尼私有格式，
            // 手上没有样本可验证，宁可明确报错也不要塞一个没跑过的解码器进来
            if (compression != 1)
            {
                res.error = $"这张是压缩 ARW（Compression={compression}），暂不支持。\n" +
                            "相机上把「RAW 文件类型」改成「未压缩」再拍即可。\n" +
            ...
            // ---- 各种电平 ----
            int black = cfa.TryGetValue(TagSonyBlack, out var bl) ? r.Int(bl, 0) : 512;
            int white = cfa.TryGetValue(TagWhiteLevel, out var wl) ? r.Int(wl, 0) : (1 << bits) - 1;
            ...
            try
            {
                res.texture = Decode(d, (int)off, w, h, black, white, pattern,
            ...
            res.info = $"{model}  {cw}×{chh}  {bits}bit 未压缩  " +
        ...
        static Texture2D Decode(byte[] d, int off, int w, int h, int black, int white, int[] pattern,
                                ...
                                Options o)
        {
            ...
            float scale = 1f;
            if (o.autoExposure)
                scale = EstimateExposure(d, off, w, h, cx, cy, cw, ch, black, range, pattern, gainR, gainB);
            ...
            for (int oy = 0; oy < outH; oy++)
            {
                ...
                for (int ox = 0; ox < outW; ox++)
                {
                    ...
                    float cr, cg, cb;
                    if (step == 1) Demosaic(d, off, w, h, rx, ry, pattern, black, range, out cr, out cg, out cb);
                    else           QuadAverage(d, off, w, h, rx, ry, step, pattern, black, range, out cr, out cg, out cb);
        ...
        /// </summary>
        static float EstimateExposure(byte[] d, int off, int w, int h,
                                      ...
                                      int[] pattern, float gainR, float gainB)
        {
            ...
            {
                float v = Mathf.Max(0f, (Raw(d, off, w, h, x, y) - black) * range);
        ...
        static int Raw(byte[] d, int off, int w, int h, int x, int y)
        {
            x = x < 0 ? 0 : (x >= w ? w - 1 : x);
            y = y < 0 ? 0 : (y >= h ? h - 1 : y);
            int i = off + ((y * w + x) << 1);
            return d[i] | (d[i + 1] << 8);      // ARW 一律小端
        }
        ...
        /// </summary>
        static void Demosaic(byte[] d, int off, int w, int h, int x, int y, int[] pattern,
                             int black, float range, out float r, out float g, out float b)
        {
            float V(int px, int py) => Mathf.Max(0f, (Raw(d, off, w, h, px, py) - black) * range);
        ...
        /// </summary>
        static void QuadAverage(byte[] d, int off, int w, int h, int x, int y, int step, int[] pattern,
                                int black, float range, out float r, out float g, out float b)
        {
            ...
            {
                float v = Mathf.Max(0f, (Raw(d, off, w, h, x + dx, y + dy) - black) * range);
