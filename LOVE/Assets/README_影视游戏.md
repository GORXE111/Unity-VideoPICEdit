...
| 在哪 | 干什么 |
|---|---|
| 组头 `◉ / ○` | 这一组要不要。**留着参数，只是先不看** |
| 组头 `S` | 独看：只看这一组。Ctrl 点可以多选几组一起看 |
| 部件行 `◉ / ○` | 单个部件的显隐。调多部件的组时最常用 |
| 顶栏 | 全显 / 全隐 / 取消独看，还有「几组生效」 |

**独看和「关掉」是两回事。** 关掉是"这组我不要了"，独看是"这会儿我只想看这组"。
混成一个的话，取消独看之后没法把你原本手动关掉的那些恢复回去。

开着、但被独看挡住的组，标题会压暗一档——否则就是「我明明开着它怎么没效果」。

> **判定只能有一份。** `VideoGradeSettings.GroupRenders` 是唯一的规则，
> 渲染器和界面都走它。分成两处写的话迟早出现「界面说 3 组生效、实际渲染 2 组」，
> 而这种偏差没人会怀疑到判定本身。

> **新加的 bool 字段，默认值必须等于「维持原样」。** 部件的显隐字段叫 `muted`
> 而不是 `enabled`：`JsonUtility` 读老预设时，文件里没有的 bool 一律是 `false`，
> 叫 `enabled` 的话所有老预设里的部件读进来全部失效，而且是静默的。

> **第一个部件恒按「加」处理，指的是第一个没被静音的那个。** 把第一个静音掉之后，
> 第二个就成了起点；那时它还按「减」算的话结果恒为全黑，界面上看就是"这组突然没了"。
...
`Tools/工具包/打包 Windows（修图台 + 视频台）`，输出在 `Build/工具包/调色工具台.exe`。
场景一样是代码生成的（`ToolAppBuild.SetupScene`），手改改完下次重建就没了。

上面一排页签切两个台，参数栏共用同一份 `GradeSettingsGUI`。

| | 修图台 | 视频台 |
|---|---|---|
| 打开 | JPG / PNG / **索尼 ARW** | mp4 / mov / mkv… |
| 预览 | 全分辨率 | 降分辨率解码（可调 240p~1440p） |
| 导出 | JPG / PNG | H.264 mp4，原始分辨率，音轨原样复制 |
| 依赖 | 无 | **ffmpeg**（找不到可以手动指定） |

**窗口化，可拖大小，F11 切全屏，窗口大小会记住**——用的是影视游戏那边同一个
`ScreenSetup`。工具全屏没法和别的软件对着用。

> `PlayerSettings` 是工程全局的，影视游戏那条线也在用同一份。
> 所以打包前存一份、打完还原——不还原的话，下次打游戏包会带着工具包的
> 产品名和窗口设置出去。

视频台把编辑器视频台踩过的坑一并带上了：

- **预览走常驻解码流**，不是「要哪一帧就起一个进程」。后者 1080p 上每帧 145~180ms，
  前者 6.4ms
- **拖时间轴只登记「想去哪一帧」**，在 `Tick` 里一拍最多兑现一次、且只兑现最后那个
- **播放不补齐落后的帧**。一次步进多帧会变成「越慢越要多解码」的正反馈
- **编码器的 stderr 必须有人读**，否则管道写满时 ffmpeg 卡死不退出
- **导出分帧走**，一口气导完窗口整个假死
- **输出尺寸开头定死**。中途改裁剪会改变尺寸，而编码器的尺寸是开头就定下的
