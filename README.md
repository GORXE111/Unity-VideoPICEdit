...
**视频台** — 独立的视频后期工具，不需要场景也不需要进 Play。任意路径的视频拖进来，
逐帧查看、调色、画质处理，按入出点导出新的 mp4。

导出走 `ffmpeg 解码 → Unity 调色 → ffmpeg 编码`，全程管道不落临时文件，音轨从源文件
直接复制。预览默认用 VideoPlayer（快、能实时播），准备不出来会自动倒向 ffmpeg 抽帧。
**需要装 ffmpeg**（自动探测 PATH 和 WinGet 目录）。
...
未压缩和 **ARW2 有损压缩（cRAW）**都支持；A1 / A7 IV 之后的无损压缩（Lossless JPEG）
暂未支持，遇到会明确报错并退回机内 JPEG 预览。
