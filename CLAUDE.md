...
- **ONNX 只能在编辑器里导入**。`Unity.Sentis.ONNX` 是编辑器程序集，出包之后没有它；
  运行时只能用已导入的 `ModelAsset`，而它**只有被场景引用着才会进包**。
- **运行时的弹出菜单是跨帧的**。回调在调用方早已返回之后才触发，
  候选名单必须先抓一份定死，不能在回调里再去遍历那会儿的状态。
- **曲线编辑里首尾两个点不能左右拖**。定义域是整个 0~1，端点挪进来之后
  外面那段没有定义，`Evaluate` 会平推出去，表现是高光或暗部整片糊死。
- **新加的 bool 字段默认值必须等于"维持原样"**。`JsonUtility` 读老数据时，
  文件里没有的 bool 一律是 `false`。所以蒙版部件的显隐字段叫 `muted` 不叫 `enabled`——
  后者会让所有老预设里的部件静默失效。
- **"要不要渲染"这类判定只能有一份**。`VideoGradeSettings.GroupRenders` 是唯一规则，
  渲染器和界面都走它；分两处写迟早出现"界面说 3 组生效、实际渲染 2 组"。
